...
const mapStateToProps = (state, ownProps) => {
  ...
  const createTableData = () => {
    ...
    return [
      ...
      addRow('Average reward points', Math.round(account.avg_reward_points), `[${Math.round(account.limits.min_avg_points_limit)}, ${Math.round(account.limits.max_avg_points_limit)}]`, `${scoreFn(3)} / ${weightsFn(3)}`),
      addRow("Reward dest. 'Staked'", !!account.reward_staked ? 'yes' : 'no', `[0, 1]`, `${scoreFn(4)} / ${weightsFn(4)}`),
      addRow("Currently Elected", !!account.active ? 'yes' : 'no', `[0, 1]`, `${scoreFn(5)} / ${weightsFn(5)}`),
      ...
      addRow("Identity judgements", account.judgements, `[${Math.abs(Math.round(account.limits.min_judgements_limit))}, ${Math.round(account.limits.max_judgements_limit)}]`, `${scoreFn(8)} / ${weightsFn(8)}`),
