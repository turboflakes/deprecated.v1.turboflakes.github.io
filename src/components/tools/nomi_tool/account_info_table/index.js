...
const mapStateToProps = (state, ownProps) => {
  ...
  const createTableData = () => {
    ...
    return [
      ...
      addRow("Sub-accounts", account.sub_accounts, `[${Math.abs(Math.round(account.limits.sub_accounts.min))}, ${Math.round(account.limits.sub_accounts.max)}]`, `${scoreFn(9)} / ${weightsFn(9)}`),
